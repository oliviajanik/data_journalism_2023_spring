...
A1: I found 60 different payments to Prince George's County schools or county the school board.

```{r}
pg_schools <- payments %>% 
filter(str_detect(vendor_name, "PRINCE GEORGES COUNTY SCHOOL") | str_detect(vendor_name, "PG COUNTY PUBLIC SCHOOL") | str_detect(vendor_name, "PRINCE GEORGES COUNTY PUBLIC SCHOOL") | str_detect(vendor_name, "BOARD OF EDUCATION OF PRINCE GEORGES COUNTY") | str_detect(vendor_name, "BD OF EDUCATION OF PG CO")
                                                                                                )
```
...
A2: The year 2022 has the most money given to the Prince George's County School System with 1,523,657,781.06. The second highest year was 2021. In these two years, schools did some virtual learning because of the pandemic, while also returning many students to classrooms by 2022, which is why I think they received the most payments.
...
A3. There are many repeats in the vendor names column in the payments data file. I believe it is because the vendors received payments in different years or in different zip codes. Some of these vendors include Wrecking Corp of American St. Louis INC, Waters Corporation, and Clark Construction Group LLC. To clean this data and to accurately see which vendor received the most money, I would combine entries that have the same company name in a new data set because we don't care about the year or zip code for this particular question.
...
A4: Stem Cell research received the most amount of money in 2009. However, it is hard to tell because there are multiple fiscal year entries with the same year, this makes it difficult to interpret the data. I am unsure why there are several entries for the same fiscal year.R Studio suggested that I use reframe instead of summarise, but I still encountered the same problem.
