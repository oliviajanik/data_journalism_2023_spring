...
**Answer** Columns in this dataframe include shape length, shape area and geometry, which we haven't seen before.
...
**Answer** It contains typical dataframe entries like zip code and name, but also entries like shape length, dimensions, area and more geometry elements.
...
**Answer** We created a map of Maryland based on zip codes.
...
### Task: Run code
**Task** Run the following code. Describe the output in the space below.
**Answer** There are 2,234 rows and 18 columns from the county datam, representing counties from all of the United States.
```{r}
...
### Task: Run code
**Task** Run the following code. Describe the output in the space below.
**Answer** We filtered the county geographic data to only give us Maryland counties.
...
### Task: Run code
**Task** Run the following code. Describe the output in the space below.
**Answer** Now we have a map of Maryland, with its county boundaries displayed. But we cannot see the bay on the map and the eastern and western parts of the state are connected because the county boundaries go into the bay.
```{r}
md_counties %>%
  ggplot() +
  geom_sf() +
...
### Task: Run code
**Task** Run the following code. Describe the output in the space below.
**Answer** We now have both the county and zip code boundaries in the map. This allows us to see the water boundaries in the bay that we could not before.
...
**Task** Run the following code. Describe the output in the space below.
**Answer** The data shows the number of foreclosures per zip code in March 2023. There are 547 zip codes, but Maryland only has 526 zips.
...
**Task** Run the following code. Describe the output in the space below.
**Answer** Now we have a dataframe with the geography and the foreclosure notices.
...
### Task: Run code
**Task** Run the following code. Describe the output in the space below.
**Answer** We have a map that shows the amount of foreclosure notices per zip code in 2023, with the lighter areas having more notices.
...
### Task: Run code
**Task** Run the following code. Describe the output in the space below.
**Answer** We have two dataframes, one of Maryland county populations and one with foreclosure notices.
...
### Task: Run code
**Task** Run the following code. Describe the output in the space below.
**Answer** Now, we have the county populations with the geometry added into the dataframe.
...
### Task: Run code
**Task** Run the following code. Describe the output in the space below.
**Answer** Maryland is in our county population, so the counties would not join together because county, Maryland is not the same thing as just county in code.
...
### Task: Run code
**Task** Run the following code. Describe the output in the space below.
**Answer** We cleaned the md_county_population so the name is the same as the notices dataframe and there is no reference to Maryland in the column.
...
### Task: Run code
**Task** Run the following code. Describe the output in the space below.
**Answer** We joined the notices and population dataframes by county.
...
### Task: Run code
**Task** Run the following code. Describe the output in the space below.
**Answer** We created a new row based on the rate of notices per 1000 people in each county.The county with the highest foreclosure rate per 1000 people is Charles County in Southern Maryland. The counties with the lowest rates are Montgomery and Howard counties.
...
### Task: Run code
**Task** Run the following code. Describe the output in the space below.
**Answer** The map confirms what we saw in the dataframe, but makes it easier to comprehend because it's visual. Charles County clearly stands out, but there are no labels to show the names of counties.
...
### Task: Run code
**Task** Run the following code. Describe the output in the space below.
**Answer** Now, the color scale is different. Yellow is the most and black is the least.
...
**Answer** This changes the color scale from 5 colors to 4 colors, still Charles County is the only yellow county.
