...
**Answer** The plot shows that the trendline between pct of trump voters and pct of businesses receiving ppp loans is positive, but it doesn't tell us how strong the relationship is.
```{r}
...
### Task 4: Plot data
**Task** Run the following code. Describe the plot. What do you see?
**Answer** The trendline here is probably skewed because of the outliers in the data. There does not appear to have a super obvious relationship.
```{r}
...
**Answer** The dataframe shows the correlation coefficient between pct of Trump votes and businesses receiving ppp loans is .52, a mild positive relationship. The correlation coefficient between pct Trump and ppp amount per employee is .22, a weak positive relationship.
...
**Answer** This code calculated the p-value for pct trump and business receiving ppp loans, it found the p-value was very low, so it is not unlikely that this happened by chance.
```{r}
...
**Task** Run the following code. Describe the model output. What do you see?
**Answer** This cor test revealed that the p-value is 0.1189, which is high for a p-value, meaning there is no way of telling that this relationship did not occur by random chance. 
```{r}
...
**Answer** The difference between Trump and Clinton voters in mean ppp amount per employee is about $300. The difference in mean businesses receiving ppp loans is about 9%.
```{r}
reveal_data %>%
  group_by(vote_2016) %>%
  summarise(
...
**Answer** The p-value is .2295, which does not give us confidence that there is a meaningful difference between Clinton and Trump voters and the amount of ppp loan per employee.
```{r}
...
### Task 10: Run t-test
**Task** Run the following code. Describe the output. What do you see?
**Answer** Here, the p-value is 0.002643 and the t-value is -3.182. This gives us reason to believe that the relationship between businesses receiving ppp loans in states and the 2016 candidate who won the state is statistically significant.
```{r}
