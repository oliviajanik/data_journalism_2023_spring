...
**Answer** The variable allows for us to hold this result for later is we want to compare just the two columns without writing over the original data.
...
**Answer** Using OR gives us classes with more than 15 seats and classes in the theatre department, not just theatre classes with more than 15 seats.
...
### Task 8: Turning a character date into a 5real date
...
**Answer:** The date is now in the year/month/day format and it is now recognized as a date, which will allow us to filter it using > or < formulas.
...
**Answer** The code rounds down all the month's data to the first of the month, showing how much money was spent each month.
...
**Answer** The counties with the highest percent of Moore votes are at the top and the spreadsheet goes from highest to lowest.
...
**Answer** No, because these counties are generally smaller and have less impact on the overall election.
...
**Answer** The code is creating a new variable where there will be an added column that shows what state the expense occurred in. It uses the string detect feature to detect address lines with the word Maryland, then says that those rows will have the value MD in the state column. It follows this pattern for other states.
...
**Answer** Yes, because it shows expenses by states. But the data could be inaccurate because sometimes street names are also state names and this could create wrong inputs for data.
...
1. Who was the most common payee on expenses of $1,000 or more?

2.  What happened to the amount of campaign expenditures in the two weeks before the primaries?

3.  What was the total sum spent on online advertising?
