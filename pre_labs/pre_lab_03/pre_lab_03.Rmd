...
**Task** Run the following codeblock to reload the data, using every row to set the data types. Does it show any parsing errors when you run? Answer below **Answer** The output does not show any errors because it checks all of the rows to determine the category type.
...
**Task** Run the following codeblock to glimpse the data. What data type is the "Vendor Name" field? What data type is the "Amount" field? What data type is the "Date" column? Answer below. **Answer** The amount column is a "double" or a number column. The date column is a character column.
...
**Answer** The data contains zip codes that start with 0, but excel and other programs don't recognize 0s at the beginning of numbers so they remove it, leading to 
...
payments <- payments %>% 
  mutate(`Vendor Zip` = ifelse(str_length(`Vendor Zip`) == 4, paste0("0" `Vendor Zip`)))
...
**Task** Run the following codeblock to count the number of payments. Looking at the result, what jumps out at you as potentially problematic? **Answer** There are categories for grants, loans, one says just "L" and one is titled contract.
...
**Answer** There are two grants that are less than 100 dollars and three that are less than 300 dollars.
...
**Task** Run the following codeblock to use str_sub() to convert the ZIP codes that have nine digits to five digits, creating a new column. Look at the difference in the result - what changed? **Answer** The zip 5 column only gives the first five digits of each zip code, making them all the same length.
...
cleaned_md_grants_loans <- md_grants_loans %>%
  ...
  mutate(zip5 = case_when(
    ...
    str_length(zip5) == 4 ~ paste0("0",zip5),
    TRUE ~ zip5
  ))
...
**Task** Write and run code to answer our original question: which zip code has gotten the most amount of money from the Maryland Tourism Board? The zip code with the most money from the Maryland Tourism Board is 21202. This zip code is located in Baltimore and neighborhoods in it include Johnstown Square, Latrobe Homes, Oldtown, Little Italy, and Harbor East. Attractions in the zip code include the aquarium, the children's museum, and the Reginald F. Lewis Museum.
