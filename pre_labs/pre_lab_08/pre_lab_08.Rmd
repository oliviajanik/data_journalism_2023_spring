...
**Answer** The first line of code is saying that we are using the baltco_911_calls data and modifying it in a new dataframe titled baltco_911_calls_by_month. The second line mutates the data by adding a month column based on the number in the date column. label= TRUE means that the number of the month will turn into an abbreviation. Then we group by month, so each month will get a separate row and summarize the total of calls for each month in the next row. Finally, we arrange the data so it goes from most to least calls.
...
**Answer** I see a blank grey rectangle.
...
**Task** Run the following code. Explain what you see after running the code in the space below.
**Answer** After running the code, I see a bar chart with 12 bars representing the months and how many calls in each month. All of the bars are charcoal grey.
...
**Answer** Now the bar chart goes from least calls to most calls rather than chronologically.
...
**Task** Run the following code. Explain what you see after running the code in the space below. What has changed from the last codeblock?
**Answer** Now the months are on the Y axis and the amount of calls are on the X axis.

```{r}
baltco_911_calls_by_month %>%
  ggplot() +
  geom_bar(aes(x=rmeorder(month,total_calls), weight=total_calls)) +
  coord_flip()
```
...
**Task** Run the following code. Explain what you see after running the code in the space below. What has changed from the last codeblock?
**Answer** It changed the background from light grey to white.
...
**Answer** Now the font size is different and the chart has a blue and white background.
```{r}
baltco_911_calls_by_month %>%
  ggplot() +
  geom_bar(aes(x=reorder(month,total_calls), weight=total_calls)) +
...
**Task** Run the following code. Explain what you see after running the code in the space below. What has changed from the last codeblock?
**Answer** The chart now has a title that reads: More 911 Overdoes Calls in Warmer Months. The X axis is now labeled number of calls and the Y axis is now labeled month. We also added the source of the data.
...
**Task** Run the following code. Explain what you see after running the code in the space below. 
**Answer** A dataframe with two columns: date and total_calls, with calls from Feb. 6, 2022 to Feb. 6, 2023.
...
**Task** Run the following code. Explain what you see after running the code in the space below. 
**Answer** The linechart looks like a heart monitor reading, with the months on the bottom serving as labels, so it is difficult to see what dates were truly outliers.
...
**Task** Run the following code. Explain what you see after running the code in the space below. 
**Answer** There is a line chart that looks like one of those heart rate monitor things, at the bottom there is the date on the X axis, which is unreadable because they are so clustered together with one week for each date break.
...
**Task** Run the following code. Explain what you see after running the code in the space below. 
**Answer** The dates are slantedl now so they are slightly easier to read, but they are still very crowded.
```{r}
baltco_911_calls_by_date %>%
  ggplot() + 
  geom_line(aes(x=date, y=total_calls)) + 
