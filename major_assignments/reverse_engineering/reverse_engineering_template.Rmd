...
* **Analysis summary**: People associated with Prestage Farms gave Moore's committee $31,200 across six payments in 2018. Three came on March 1, 2018, which is about two and a half months before the session began. One donation occurred on June 28, 2018 and the final two occurred on June 30, 2018.

```{r}
# Put code to reverse engineer sentence here
NC_donors_5 <- cleaned_NC_donors %>%
  filter(str_detect(employer_name, "PRESTAGE FARMS") & str_detect(committee_name, "MOORE")) %>%
  group_by(name_clean, date) %>% 
  reframe(amount) %>%
  arrange(date) %>%
  adorn_totals
